...
contract AuthBank {
    ...
    function revokeAuth(address tokenAddress, uint256 tokenId) external {
        require(
            getTokenAuthIndex(tokenAddress, tokenId) != 0,
            "AuthBank: No Auth found for this token"
        );

        uint256 authIndex = getTokenAuthIndex(tokenAddress, tokenId);

        auths[msg.sender][authIndex].isAuth = false;
        emit AuthRevoked(tokenAddress, tokenId, msg.sender);
    }

    function getTokenAuthIndex(address tokenAddress, uint256 tokenId)
        public
        view
        returns (uint256)
    {
        for (uint256 i = 1; i <= _authCounters[msg.sender].current(); i++) {
            if (
                auths[msg.sender][i].tokenAddress == tokenAddress &&
                auths[msg.sender][i].tokenId == tokenId &&
                auths[msg.sender][i].isAuth == true
            ) {
                return i;
            }
        }
